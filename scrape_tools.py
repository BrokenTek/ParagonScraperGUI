...
from scrape_seats import scrapeSeats
...
# Returns three lists of the same length. The second two lists contain interior lists that are the same
#	length of eachother by index. These lists can be traversed to print or write the data in a format
#	that is best for Paragon Theaters projection of customer volume.
def getFormattedLists(list_of_movie_objs):
	movie_names = []
	...
	return movie_names, list_of_times_of_movie, list_of_sold_seats
...
def printInFormat(list_of_movie_objs):
	movie_names, list_of_times_of_movie, list_of_sold_seats = getFormattedLists(list_of_movie_objs)
...
def getDataGui():
	f = open('lux_box_seats.txt', 'r')
	seat_data = f.readlines()
	f.close()
	seat_data_str = ""
	for item in seat_data:
		seat_data_str += item.strip()
		seat_data_str += "\n"
	return seat_data_str

''' MOVED TO gui.py TO DISPLAY A PROCESSING MSG
def runScraper():
	# FOR GUI
	print("Processing")
	movies = getMovies()
